...
func SpreadBP(bestBid, bestAsk float64) float64 {
	...
	return (bestAsk - bestBid) / ((bestAsk + bestBid) / 2) * 10_000
}
...
func BuySlippageBP(book domain.OrderBookSnapshot, amountUSD float64) float64 {
	return slippageBP(book.Asks, amountUSD, true)
}

func SellSlippageBP(book domain.OrderBookSnapshot, amountUSD float64) float64 {
	return slippageBP(book.Bids, amountUSD, false)
}
...
func slippageBP(levels []domain.Level, amountUSD float64, buy bool) float64 {
	if len(levels) == 0 || amountUSD <= 0 {
		return 0
	}
	remaining := amountUSD
	...
	avg := cost / qty
	if buy {
		return (avg - best) / best * 10_000
	}
	return (best - avg) / best * 10_000
}
