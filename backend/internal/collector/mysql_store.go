...
func applyActivitySchemaPostInit(db *sql.DB) error {
	sourceStateColumns := []struct {
		...
		{name: "producer_watermark_at", after: "last_success_at", definition: "DATETIME(3) NULL"},
		{name: "bootstrap_completed_at", after: "producer_watermark_at", definition: "DATETIME(3) NULL"},
	}
	for _, col := range sourceStateColumns {
	...
	eventColumns := []struct {
		...
		{name: "source_observed_at", after: "rich_fields_summary_json", definition: "DATETIME(3) NULL"},
		{name: "source_producer_watermark_at", after: "source_observed_at", definition: "DATETIME(3) NULL"},
		{name: "source_bootstrap_completed_at", after: "source_producer_watermark_at", definition: "DATETIME(3) NULL"},
	}
	for _, col := range eventColumns {
		if err := ensureColumnExists(db, "t_activity_event", col.name, col.definition, col.after); err != nil {
			return err
		}
	}
	return nil
