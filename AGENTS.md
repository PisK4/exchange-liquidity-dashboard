...
## Listing Agent Top30 Lark Push

The Top30 hot-gap Lark alert pipeline (CoinGecko Top30 → `t_top30_snapshot` → `t_listing_delivery_outbox` → 飞书 webhook) is fully documented in `docs/feat/listing-agent-top30-hot-gap-push.md`. Read that doc before editing anything in `backend/internal/listing/`. Contracts to be aware of:
