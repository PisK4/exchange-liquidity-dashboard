...
import { bp, getFrontendURLLookup, getJSON, money, pct, type PlatformRow } from '@/lib/api/client';
...
export default async function LiquidityPage({ searchParams }: { searchParams: { symbol?: string } }) {
  ...
  const [data, lookup] = await Promise.all([
    getJSON<Liquidity>(`/api/snapshot/liquidity?symbol=${encodeURIComponent(symbol)}`),
  ...
  return <>
    ...
    <div className="grid">
      ...
      <section className="panel span-6"><h2>Status</h2><table><tbody>{data.rows.map(r => <tr key={r.platform}><td><PlatformCell platform={r.platform} displaySymbol={symbol} lookup={lookup} /></td><td><StatusBadge status={r.depth_status} reason={r.partial_reason} /> {r.data_freshness === 'delayed' ? <span className="badge delayed">delayed</span> : null}</td><td className={r.data_freshness === 'delayed' ? 'muted' : 'error'}>{r.data_freshness === 'delayed' ? `last collection ${r.last_collection_status}: ${r.last_collection_error ?? ''}` : r.error}</td></tr>)}</tbody></table></section>
      <section className="panel span-12"><h2>Platform × Tier</h2><table><thead><tr><th>Platform</th><th>0.05%</th><th>0.10%</th><th>1.00%</th><th>2.00%</th><th>Status</th><th>Freshness</th><th>Source</th></tr></thead><tbody>{data.rows.map(r => <tr key={r.platform}><td><PlatformCell platform={r.platform} displaySymbol={symbol} lookup={lookup} /></td><td>{money(r.depth_by_tier?.['0.05%']?.total_usd)}</td><td>{money(r.depth_by_tier?.['0.10%']?.total_usd)}</td><td>{money(r.depth_by_tier?.['1.00%']?.total_usd)}</td><td>{money(r.depth_by_tier?.['2.00%']?.total_usd)}</td><td><StatusBadge status={r.depth_status} reason={r.partial_reason} /></td><td>{r.data_freshness === 'delayed' ? <span className="badge delayed">delayed</span> : <span className="muted">live</span>}</td><td className="muted">{r.source_endpoint}</td></tr>)}</tbody></table></section>
    </div>
  </>;
}
