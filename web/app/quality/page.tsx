...
import { bp, getJSON, type PlatformRow } from '@/lib/api/client';
...
export default async function QualityPage({ searchParams }: { searchParams: { symbol?: string; bucket?: string } }) {
  ...
  const data = await getJSON<Quality>(`/api/snapshot/quality?symbol=${encodeURIComponent(symbol)}`);
  ...
  return <>
    ...
    <div className="grid">
      ...
      <section className="panel span-12"><h2>Quality Detail</h2><table><thead><tr><th>Platform</th><th>Spread</th><th>Imbalance</th><th>Buy</th><th>Sell</th><th>Status</th><th>Freshness</th></tr></thead><tbody>{data.rows.map(r => <tr key={r.platform}><td>{r.platform}</td><td>{bp(r.spread_bp)}</td><td>{r.imbalance_pct?.toFixed(2) ?? '-'}</td><td>{bp(r.buy_slippage_bp?.[bucket])}</td><td>{bp(r.sell_slippage_bp?.[bucket])}</td><td><StatusBadge status={r.depth_status} reason={r.partial_reason} /></td><td>{r.data_freshness === 'delayed' ? <span className="badge delayed">delayed</span> : <span className="muted">live</span>}</td></tr>)}</tbody></table></section>
    </div>
  </>;
}
