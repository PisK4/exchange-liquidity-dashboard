  ...
  return (
    <>
      ...
      <header className="topbar">
        ...
        {data.meta.data_sources?.coingecko?.enabled ? (
          <span className="meta" title={`CoinGecko · 拉取间隔 ${data.meta.data_sources?.coingecko?.pull_interval ?? '?'}${data.meta.data_sources?.coingecko?.last_pull_ts ? ` · 上次成功 ${new Date(data.meta.data_sources?.coingecko?.last_pull_ts ?? '').toLocaleString('zh-CN', { hour12: false })}` : ''}`}>
            数据源 · CoinGecko ✓
          </span>
        ) : null}
      </header>
  ...
  return (
    <div className="page-content active">
      <section className="panel">
        ...
        {data.status === 'unsupported' ? <StatusEmptyState status="unsupported" message={data.platform === 'edgeX' ? 'edgeX 暂未通过 CoinGecko 暴露 Top30 数据' : 'CoinGecko 尚未返回该平台 Top30 排行，等待下一次拉取'} /> : null}
        <div className="table-wrap"><table className="tbl"><thead><tr><th className="num">#</th><th>Symbol</th><th>来源</th><th className="num">24h Vol</th><th className="num">7d Vol</th><th className="num">7d Δ</th><th className="num">edgeX 已上线?</th><th className="num">竞品 Top30 覆盖</th><th>建议动作</th></tr></thead><tbody>{data.rows.map(row => <tr key={`${row.rank}-${row.symbol}`}><td className="num">{row.rank}</td><td>{row.symbol}</td><td>{row.data_source === 'coingecko' ? <span className="muted">CoinGecko</span> : row.data_source === 'native' ? <span className="muted">原生</span> : <span className="muted">—</span>}</td><td className="num">{row.status === 'unsupported' ? '—' : money(row.volume_24h_usd)}</td><td className="num">{typeof row.volume_7d_usd === 'number' ? money(row.volume_7d_usd) : <StatusBadge status={row.volume_7d_status ?? 'unsupported'} />}</td><td className="num">{typeof row.delta_7d_pct === 'number' ? pct(row.delta_7d_pct) : <StatusBadge status={row.delta_7d_status ?? 'unsupported'} />}</td><td className="num">{row.edgex_listed_status ? <StatusBadge status={row.edgex_listed_status} /> : row.edgex_listed ? '是' : '否'}</td><td className="num">{row.competitor_top30_coverage_status ? <StatusBadge status={row.competitor_top30_coverage_status} /> : row.competitor_top30_coverage}</td><td>{row.suggested_action_status ? <StatusBadge status={row.suggested_action_status} /> : row.suggested_action}</td></tr>)}</tbody></table></div>
      </section>
    </div>
  );
}
